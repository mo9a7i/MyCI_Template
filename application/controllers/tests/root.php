...
class Root extends CI_Controller{
	...
	function reset_all_users($offset = 0)
	{
		...
		foreach($results as $user)
		{
			...
			$this->mo9->send_email("F6sny.com فطسني.كوم" , "admin@f6sny.com" , $user->email, "فطسني.كوم عاد إليكم من جديد", $message);
		}
	...
	function create_sitemap()
	{
		...
		$posts = $this->mo9->get_posts(array('status_id'=>1,'nolimit'=>TRUE,'include_all'=>TRUE));
		...
		$pages = $this->mo9->get_pages(array('status_id'=>1,'nolimit'=>TRUE,'include_all'=>TRUE));
		...
		$members = $this->mo9->get_users(array('status_id'=>1,'nolimit'=>TRUE,'include_all'=>TRUE));
	...
	function set_posts_slugs()
	{
		...
		foreach ($posts as $post)
		{
			if($post->slug == "null")
			{
				$slug = $this->mo9->create_slug($post->content);
				//echo $slug; die();
				$this->db->where('posts.id', $post->id);
	...
	function reset_posts_titles()
	{
		...
		foreach ($posts as $post)
		{
			if($post->title == "null")
			{
				$title = $this->mo9->create_slug($post->content);
	...
	function test_sanitize()
	{
		...
		echo $this->mo9->sanitize_text($text);
		echo "<br>";
		$slug = $this->mo9->create_slug($text);
		echo $slug;
		echo "<br>";
		echo $this->mo9->get_unique_slug(array('table'=>'posts','slug'=>$slug));
	}
	...
	function migrate()
	{
		...
		foreach ($profs as $prof)
		{
			...
			$this->mo9->add_meta($meta);
			$this->mo9->link_departments_institutes($meta['institute_id'],$meta['department_id']);
		...
		foreach ($news as $new)
		{
			...
			$data = array(
				...
				$image_id = $this->mo9->add_images($data);
