...
class Mo9 extends CI_Model{
	...
	{
		$moderated_ips = $this->mo9->get_moderation_ips();
	...
	{
		$posts = $this->mo9->get_posts(array('status_id'=>4,'nolimit'=>true));
	...
	function tweet($post_id)
	{
		if(!is_null($post_id))
		{
			$tweet_text = $this->mo9->get_posts(array('post_id'=>$post_id));
			$tweet_text = $tweet_text[0];
			$my_text = $tweet_text->content;
			$my_text = $this->mo9->sanitize_text($my_text);
