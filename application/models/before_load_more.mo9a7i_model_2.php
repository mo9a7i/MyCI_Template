...
class Mo9a7i_model extends CI_Model{
	...
	function get_users($options = array())
	{
		...
		{
			//$user
			if (isset($options['user_view']) AND $options['user_view'] ) $this->db->where('users.active', 1);
		}
	...
	{
		$this->db->where('id', $id);
		return $this->db->update('users', array('status_id' => 3)); 
	}
	...
	{
		$options = $this->_default(array('order_column' =>'posts.date_modified','order_direction'=> 'desc'), $options);
	...
	{
		#Default Values
		$options = $this->_default(array('resource_type'=>2,'status_id'=>1,'order_column' =>'replies.id','order_direction'=> 'desc'), $options);
	...
	function get_author_link($reply)
	{
		if($reply->user_id==0)
			...
			if($reply->author_url)
				$author_link = '<a href="'.$reply->author_url.'" class="author-link">'.$reply->author.'</a>';
			else
				$author_link = $reply->author;
		else
			$author_link = '<a href="'.base_url().'members/'.$reply->username.'" class="author-link">'.$reply->username.'</a>';
	...
	{
		if (!is_null($id)) $this->db->where('pages.id', $id);
		else
		{
			if(is_numeric($this->uri->segment($this->pagination->uri_segment))  OR  ($this->uri->segment($this->pagination->uri_segment)) == null)
	...
	function tweet($post_id)
	{
		if(!is_null($post_id))
		{
			...
			if(mb_strlen($my_text,'utf-8') >140)
			{
				return "error1";
			}
			else
